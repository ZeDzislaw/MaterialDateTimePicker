...
public class TimePickerDialog extends DialogFragment implements
    ...
    public static TimePickerDialog newInstance(OnTimeSetListener callback,
                                               int hourOfDay, int minute, int second, boolean is24HourMode, Locale defaultLocale) {
        TimePickerDialog ret = new TimePickerDialog();
        ret.initialize(callback, hourOfDay, minute, second, is24HourMode, defaultLocale);
        return ret;
    }

    public static TimePickerDialog newInstance(OnTimeSetListener callback,
                                               int hourOfDay, int minute, boolean is24HourMode, Locale defaultLocale) {
        return TimePickerDialog.newInstance(callback, hourOfDay, minute, 0, is24HourMode, defaultLocale);
    }

    @SuppressWarnings("unused")
    public static TimePickerDialog newInstance(OnTimeSetListener callback, boolean is24HourMode, Locale defaultLocale) {
        Calendar now = Calendar.getInstance();
        return TimePickerDialog.newInstance(callback, now.get(Calendar.HOUR_OF_DAY), now.get(Calendar.MINUTE), is24HourMode, defaultLocale);
    }
    ...
    public void initialize(OnTimeSetListener callback,
        ...
        TimePickerDialog.defaultLocale = defaultLocale;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        if (!mTitle.isEmpty()) {
            ...
            timePickerHeader.setText(mTitle.toUpperCase(defaultLocale));
        }
    ...
    private void setMinute(int value) {
        ...
        CharSequence text = String.format(defaultLocale, "%02d", value);
        Utils.tryAccessibilityAnnounce(mTimePicker, text);
    ...
    private boolean addKeyIfLegal(int keyCode) {
        ...
        Utils.tryAccessibilityAnnounce(mTimePicker, String.format(defaultLocale, "%d", val));
    ...
    private int getAmOrPmKeyCode(int amOrPm) {
        ...
        if (mAmKeyCode == -1 || mPmKeyCode == -1) {
            ...
            for (int i = 0; i < Math.max(mAmText.length(), mPmText.length()); i++) {
                amChar = mAmText.toLowerCase(defaultLocale).charAt(i);
                pmChar = mPmText.toLowerCase(defaultLocale).charAt(i);
                if (amChar != pmChar) {
